...
<template>
    <AuthenticatedLayout :title="title">
        <div class="row">
            <div class="col-lg-3 " v-if="show_form">
                <div class="sticky-sidebar ">
                    <form action="" class="card card-body" @submit.prevent="handleSubmit">
                        ...
                        <div class="row">
                            ...
                            <div class="col-6">
                                <div class="form-check form-switch">
                                    <input v-model="form.roles" disabled class="form-check-input" type="checkbox"
                    ...
                    <div class="d-flex justify-content-between mb-3">
                        <div v-if="id.length > 0">
                            <Link method="put" :preserve-state="false" :href="route('admin.users.bulk-actions')"
                                :data="{ status: 1, id }" class="btn btn-sm me-1 btn-primary "><i
                                class="fa fa-undo me-1"></i>
                            Unblock</Link>
                            <Link method="put" :preserve-state="false" :href="route('admin.users.bulk-actions')"
                                :data="{ status: 0, id }" class="btn me-1 btn-sm text-primary "><i
                        ...
                        <table class="table table-hover table-sm small">
                            ...
                            <tbody>
                                <tr v-for='user in users.data' :key="user.id">
                                    ...
                                    <td>
                                        <i v-if="user.roles.includes('admin')"
                                            class="fas fa-fire-flame-curved px-1 text-danger" title="Administrator"></i>
                                    ...
                                    <td class="text-nowrap">
                                        ...
                                        <Link preserve-scroll v-if="user.status" class="btn p-0 "
                                            ...
                                            method="put"><i class="fa fa-check-circle me-1 text-success"></i>Active
                                        </Link>

                                        <Link preserve-scroll v-else class="btn p-0"
                                            :href="route('admin.users.bulk-actions')" :data="{ status: 1, id: [user.id] }"
                                            method="put"><i class="fa fa-times-circle me-1  text-danger"></i>
                                        Block</Link>
                                    </td>
