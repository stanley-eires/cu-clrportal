...
let handleSubmit = ( action ) => {
    ...
    form.transform( ( data ) => ( {
        ...data, action
    } ) ).post( route( 'admin.course.save', [ props.course.id ?? null ] ), { preserveState: true } )
}
let addActivity = ( ev ) => {
    ...
    <AuthenticatedLayout :title="title">
        <div class="mb-3   py-2 card-header">
            ...
            <button type="submit" @click.prevent="handleSubmit('close')"
                class="btn border me-1 border-primary text-primary "><i class="fa fa-check me-1"></i> Save &
                Close</button>
            <button type="submit" @click.prevent="handleSubmit('new')"
                class="btn border me-1 border-primary text-primary "><i class="fa fa-plus me-1"></i> Save &
                New</button>
        ...
        <form class="row">
                    ...
                    <div class="col-lg-12">
                        <ul class="nav nav-tabs separator-tabs mb-2">
                            ...
                            <li class="nav-item" role="presentation">
                                <button class="nav-link" data-bs-toggle="tab" data-bs-target="#res-topic"
                                    type="button">Resource By Topics</button>
                            </li>
                            <li class="nav-item" role="presentation">
                                <button class="nav-link" data-bs-toggle="tab" data-bs-target="#res-skill"
                                    type="button">Resource By Skills</button>
                            </li>
                        </ul>
                        <div class="tab-content">
                            ...
                            <div class="tab-pane " id="res-skill">
                                <div class="accordion" id="activity">
                                    <module-activity label="Skillset" :model-value="resource"
                                        :id="`res-skill-${resource.id}`"
                                        v-for="(resource, index) in form.resource_by_skills" :index="index"
                                        :key="resource.id" @removeActivity="removeActivity"></module-activity>
            ...
            <div class="col-lg-4">
                ...
                <div v-if="form.course_banner" class="d-flex align-items-center alert-light rounded-0  p-3">
                    <img class="me-2 img-thumbnail rounded-1" :src="banner"
